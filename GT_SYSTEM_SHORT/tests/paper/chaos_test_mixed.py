...
# A80 (2026-06-12): import from the MIXED stress driver — 16 equities +
# 16 FX pairs in one fleet. stress_churn_mixed.py routes asset-specific
# ops (LTP/flatten/tick/offset) per-symbol on an `asset` tag.
from tests.paper.stress_churn_mixed import (
...
# A80: the mixed fleet holds BOTH equity (secType STK) and FX (secType
# CASH) positions/orders. Broker-truth must inspect both. This helper
# returns the canonical stress symbol for any contract — the bare STK
# symbol, or the FX pair with the IDEALPRO dot stripped (EUR.USD →
# EURUSD) — and '' for anything outside our universe.
def _canon_symbol(c) -> str:
    sec = getattr(c, "secType", "") or ""
    if sec == "STK":
        return getattr(c, "symbol", "") or ""
    if sec == "CASH":
        ls = (getattr(c, "localSymbol", "") or "").replace(".", "")
        return ls or (getattr(c, "symbol", "") or "")
    return ""
...
async def _scenario_rolling_kill(
    ...
    # A81: three-truths reconciliation (engine · broker · market). The
    # broker-truth check above proves the account is FLAT; this proves all
    # THREE views agree symbol-by-symbol — the deepest safety invariant.
    await _three_truths_gate(PORT)
...
async def _three_truths_gate(port: int) -> None:
    """Run the engine·broker·market reconciliation and print the verdict.
    Imported lazily so chaos_test_mixed still loads if scripts/ is absent."""
    try:
        import sys as _sys
        scripts_dir = str(PROJECT_ROOT / "scripts")
        if scripts_dir not in _sys.path:
            _sys.path.insert(0, scripts_dir)
        import three_truths as _tt
        report = await _tt.run(port, symbols=set(_STRESS_SYMS), check_ltp=True)
        _tt._print(report, check_ltp=True)
    except Exception as e:
        print(f"  [three-truths] reconciliation skipped: {type(e).__name__}: {e}")
