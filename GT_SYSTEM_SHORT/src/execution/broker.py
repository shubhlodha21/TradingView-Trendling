...
class Gateway:
    ...
    __slots__ = (
        ...
        # Two independent reconnectors call connect() on this Gateway (the
        # ConnectionManager supervisor via _reconnect_gateway, and the engine
        # health loop via _try_active_reconnect). They don't know about each
        # other. Coalescing here — rather than coordinating them — means a
        # second caller joins the in-flight handshake instead of starting a
        # rival one against the same clientId.
        '_connect_task',
        # IBKR's own daily P&L (reqPnL → pnlEvent.dailyPnL). Authoritative,
        # auto-resets each trading day. Risk gate's daily-loss circuit
        # breaker gates on this when available.
        '_pnl_daily', '_pnl_realized', '_pnl_unrealized',
        '_pnl_known', '_pnl_subscribed',
    ...
    def __init__(
        ...
        self.symbol = symbol
        self.paper = paper
        # --cfd: resolve the GENERIC CFD variant of `symbol` (via
        # hint=AssetClass.CFD) instead of the equity/spot contract. Default
        # False → byte-identical legacy resolution (equity/FX untouched).
        self.cfd = cfd
        ...
        # IBKR's authoritative daily P&L (realized + unrealized for the
        # current trading day), pushed via pnlEvent. Auto-resets each session
        # at IBKR. `_pnl_known` gates "have we received a value yet".
        self._pnl_daily: float = 0.0
        self._pnl_realized: float = 0.0     # today's realized (closed trades)
        self._pnl_unrealized: float = 0.0   # total open-position mark-to-market
        self._pnl_known: bool = False
        self._pnl_subscribed: bool = False
    ...
    async def _connect_inner(self) -> bool:
        ...
        try:
            ...
            if old_ib is not None:
                ...
                # Same for reqPnL. _subscribe_pnl() early-returns on this
                # flag, and nothing else ever clears it — so without this
                # reset the NEW IB instance never subscribes, and
                # _pnl_daily/_pnl_realized stay frozen at their
                # pre-disconnect values for the life of the process while
                # the risk gate's daily-loss breaker keeps reading them.
            ...
            # IBKR's own daily P&L feed — the risk gate's daily-loss circuit
            # breaker prefers this authoritative, auto-resetting number.
    ...
    async def _get_contract(self, exchange="SMART", currency="USD"):
        ...
        # Fallback sweep if primary fails (handles symbols outside the
        # override list without silent breakage). NOT in CFD mode — equity
        # venue fallbacks would qualify the wrong (equity) instrument.
    ...
    async def flatten_position(self, symbol: str) -> Optional[dict]:
        ...
        # tif='GTC' per operator preference (GTC everywhere). A market order
        # fills instantly so TIF is cosmetic; see place_order() for the
        # GTC-on-MKT caveat.
        order = MarketOrder(
            action=side, totalQuantity=qty, tif='GTC',
            outsideRth=True,
        )
    ...
    def register_existing_order(
        ...
        #
        # SHORT INVERSION (P11): bracket parent is the SELL entry, child is
        # the BUY cover. Determine role + peer from the engine_id naming
        # convention (matches `_make_engine_id`):
        #     ENTRY_SELL_* → bracket parent — peer is the child BUY STP cover
        #                    with `parentId == self.orderId`
        #     BR_BUY_*     → bracket child  — peer is `order.parentId`
        # Anything else (SL_*, ENTRY_LMT_*, etc.) is not part of a
        # bracket pair and gets no lifecycle logger attached.
        # (The old long form keyed on ENTRY_BUY_ / child action SELL / BR_SELL_
        # — for a short NEITHER branch matched, so adopted brackets never got
        # the A57 child-cancel⇒kill-orphan-parent logger re-attached and a
        # cancelled adopted child could leave the resting SELL parent alive →
        # it later fires into a fresh NAKED SHORT.)
        try:
            role = None
            peer_broker_id = ''
            if isinstance(engine_id, str):
                if engine_id.startswith('ENTRY_SELL_'):
                    role = 'PARENT'
                    this_id = int(trade_obj.order.orderId)
                    # Walk trades to find the child (BUY STP cover with
                    ...
                    for t in self._ib.trades():
                        try:
                            ...
                            continue
                elif engine_id.startswith('BR_BUY_'):
                    role = 'CHILD'
    ...
    async def _live_order(
        ...
        # Operator preference (2026-07-07): GTC on EVERYTHING, including the
        # instant-fill MARKET orders. A market order fills immediately so its
        # TIF is cosmetic — but NOTE some IBKR account types reject GTC on MKT
        # orders; if force_exit / square_off ever fails to place, this is why,
        # and the account's order preset should be set to GTC (or this line
        # reverted to DAY).
        if order_type == OrderType.LIMIT and limit_price:
            order = LimitOrder(
            ...
            order = MarketOrder(
                action=side.value, totalQuantity=qty, tif='GTC',
                outsideRth=True,
            )
    ...
    async def place_bracket_sell_stop_market(
        self,
        qty: int,
        ...
        """SHORT INVERSION (P1/P3): submit a SELL STP-LMT parent (breakdown
        entry) + BUY STP (market on trigger) child (protective cover) as a
        single IBKR bracket. The child BUY rests at the broker the instant
        the bracket is accepted, even though it's marked "Pending Submit"
        until the parent fills — so a disconnect or crash between submit and
        fill leaves the SHORT position protected at the broker, not by the
        engine. (Long version was BUY parent + SELL child.)
        ...
        # PARENT: SELL STP-LMT (breakdown entry), transmit=False (buffered
        # until child arrives). IBKR requires lmtPrice <= stopPrice for a
        # SELL stop-limit — the engine sets limit = stop - offset, which
        # satisfies that (mirror of the long BUY constraint lmt >= stop).
        #
        # `parent_market=True` (config.entry_market / --market) swaps the
        # parent for a plain SELL MARKET: the breakdown was already detected
        # upstream, so there is no trigger left to wait for. Everything else
        # about the bracket is unchanged.
        if parent_market:
        ...
        # CHILD: BUY STP (market on trigger) — protective COVER for the
        # short. parentId = parent's id. `transmit=True` flushes both to
        # IBKR atomically. NO ocaGroup — see big comment above.
        child = StopOrder(
        ...
        # dispatches by `order.side` from the registry. SHORT regime: the
        # parent is SELL so parent fills land in the SELL (open-short)
        # branch, and the child is BUY so child fills land in the BUY
        # (cover/close) branch — no additional discriminator needed here.
    ...
    async def modify_stop_trigger(
        ...
        if self.paper:
            ...
            for pid, info in list(self._pending_limits.items()):
                # SHORT INVERSION (P11): the protective bracket child being
                # retargeted is the BUY cover (was SELL in the long form), so
                # the paper post-fill modify never found it → child kept its
                # pre-VWAP trigger.
                if pid == order_id and info.get('side') == OrderSide.BUY:
...
"""
    ...
    async def _subscribe_pnl(self) -> None:
        """Subscribe to IBKR's account P&L (`reqPnL`) at connect.

        IBKR computes `dailyPnL` itself — realized + unrealized change for
        the current trading day — and auto-resets it each session. We mirror
        it into `_pnl_daily` via `pnlEvent` so the risk gate's daily-loss
        circuit breaker can gate on IBKR's authoritative number instead of a
        home-grown cumulative realized sum (which never reset daily).

        Best-effort + idempotent. On any failure the risk gate falls back to
        its internal computation. ZERO hot-path cost — the value is pushed by
        IBKR and read as a plain attribute.
        """
        if self._pnl_subscribed or self._ib is None:
            return
        try:
            # reqPnL needs the account code.
            accounts = list(self._ib.managedAccounts() or [])
            account = accounts[0] if accounts else ""
            if not account:
                print("[Gateway] PnL subscribe skipped: no managed account yet")
                return
            pnl_obj = self._ib.reqPnL(account)  # PnL object, updated in place
            ...
            def _on_pnl(p):
                try:
                    ...
                    def _clean(v):
                        # IBKR sends NaN / a huge sentinel (~1.7e308) before
                        # the first real value — reject those.
                        if v is None:
                            return None
                        f = float(v)
                        return f if (not _m.isnan(f) and abs(f) < 1e12) else None

                    d = _clean(getattr(p, 'dailyPnL', None))
                    if d is not None:
                        self._pnl_daily = d
                        self._pnl_known = True
                    r = _clean(getattr(p, 'realizedPnL', None))
                    if r is not None:
                        self._pnl_realized = r
                    u = _clean(getattr(p, 'unrealizedPnL', None))
                    if u is not None:
                        self._pnl_unrealized = u
                except (ValueError, TypeError):
                    pass

            try:
                self._ib.pnlEvent += _on_pnl
            except Exception:
                pass
            _on_pnl(pnl_obj)  # seed if already populated
            self._pnl_subscribed = True
            print(f"[Gateway] Account PnL subscribed (account={account})")
        except Exception as e:
            print(f"[Gateway] PnL subscribe failed: {type(e).__name__}: {e}")

    def get_daily_pnl(self) -> Optional[float]:
        """IBKR's authoritative daily P&L (realized + unrealized for the
        trading day), or None when not yet known. Auto-resets each session
        at IBKR. The risk gate's daily-loss circuit breaker prefers this
        over the internal cumulative realized sum. Pure attribute read —
        no IBKR call, safe on the hot path.
        """
        if not self.connected or not self._pnl_known:
            return None
        return self._pnl_daily

    def get_realized_pnl(self) -> Optional[float]:
        """IBKR's daily REALIZED P&L (today's closed trades, account-wide),
        or None when not yet known. Excludes open-position mark-to-market."""
        if not self.connected or not self._pnl_known:
            return None
        return self._pnl_realized

    def get_unrealized_pnl(self) -> Optional[float]:
        """IBKR's total UNREALIZED P&L (open positions' mark-to-market),
        or None when not yet known."""
        if not self.connected or not self._pnl_known:
            return None
        return self._pnl_unrealized

    def get_day_pnl_combined(self) -> Optional[float]:
        """Operator's chosen daily-loss basis: realizedPnL + unrealizedPnL —
        today's closed-trade P&L plus open positions' total mark-to-market.
        This is what the $2k daily-loss gate and the dashboard 'day P&L' pill
        use. None until the reqPnL feed has landed."""
        if not self.connected or not self._pnl_known:
            return None
    ...
    def get_excess_liquidity(self) -> float:
        """Excess Liquidity (`ExcessLiquidity` from accountSummary) — the
        buffer above maintenance margin. This is the number that must
        stay positive for a short to be safe; short-sale proceeds are
        already excluded from it by IBKR (PDF §3), so it is the correct
        authoritative gate rather than a home-grown cash calculation.
        Returns 0.0 (= "unknown") when not yet pushed.
        """
        return self.get_account_value("ExcessLiquidity")

    async def whatif_order_margin(
        self,
        action: str,
        quantity,
        order_type: str = "MKT",
        ...
        """IBKR Order Preview (whatIf) — the AUTHORITATIVE margin impact
        of an order WITHOUT placing it (PDF §6).

        For a short, the offline `ShortPolicy` is only a conservative
        estimate; the real initial/maintenance margin is stock-specific
        and changes daily. `whatIf=True` asks IBKR to compute the
        OrderState and return it WITHOUT submitting — nothing is placed,
        so this is read-only and safe to call before arming a short.

        Args:
            action: "SELL" to preview opening a short (or "BUY" to cover).
            quantity: order size (shares / contracts / base units). abs()'d.
            order_type: "MKT" is fine for a margin preview.
            contract: optional; defaults to the gateway's qualified contract.

        Returns a dict of floats (in the account base currency IBKR
        reports in), or None if unavailable (paper, disconnected, or the
        call failed — callers fall back to the offline estimate):
            init_margin, maint_margin  — the *change* this order causes
            init_margin_after, maint_margin_after
            equity_with_loan_after     — equity after the trade (PDF §3/§6)
            commission
        """
        ...
        try:
            ...
            c = contract if contract is not None else self._contract
            if c is None:
                c = await self._get_contract()

            # A MKT whatIf frequently returns EMPTY margin — IBKR has no price
            # to margin the position against, so initMargin*/maintMargin* come
            # back blank (the "margin shows offline (est)" symptom). Prefer a
            # LIMIT order at a REAL price so the preview actually computes.
            # Resolve a price: caller-provided → gateway's last cached tick →
            # a quick market-data snapshot.
            px = float(price) if price else 0.0
            if px <= 0:
                px = float(getattr(self, '_last_price', 0) or 0)
            if px <= 0:
                try:
                    import math as _m, time as _t
                    tk = self._ib.reqMktData(c, snapshot=True)
                    _dl = _t.monotonic() + 2.5
                    while _t.monotonic() < _dl:
                        ...
                        if last is not None and not _m.isnan(last) and last > 0:
                            px = float(last); break
                        if (bid and ask and not _m.isnan(bid) and not _m.isnan(ask)
                                and bid > 0 and ask > 0):
                            px = (float(bid) + float(ask)) / 2; break
            ...
            # TIF=GTC (operator preference). Setting it explicitly also avoids
            # the account "order preset" filling in DAY unprompted (Error
            # 10349). NOTE: if the preset is a HARD-force type it can still
            # rewrite GTC→DAY and re-raise 10349 — but a LIMIT-priced whatIf
            # (below) computes margin regardless of that TIF warning.
            order.tif = "GTC"
            if px > 0:
                order.orderType = "LMT"
                order.lmtPrice = round(px, 2)
            else:
                order.orderType = order_type  # fallback (MKT) — may return empty
            order.whatIf = True  # ← compute only; DO NOT place
            state = await self._ib.whatIfOrderAsync(c, order)
            if state is None:
                return None
            ...
            init_change = _f(getattr(state, "initMarginChange", None))
            maint_change = _f(getattr(state, "maintMarginChange", None))
            init_after = _f(getattr(state, "initMarginAfter", None))
            maint_after = _f(getattr(state, "maintMarginAfter", None))
            # IBKR (notably PAPER accounts) frequently leaves the margin
            # *Change* fields empty on a whatIf while still populating the
            # *After* totals. From a FLAT account the After margin ≈ this
            # order's margin, so fall back to it rather than dropping to the
            # offline estimate — that's why init/maint showed "(est)" while
            # availability/borrow came through live.
            init_margin = init_change if init_change is not None else init_after
            maint_margin = maint_change if maint_change is not None else maint_after
            # One-line diagnostic (stderr → won't corrupt the TUI) so an
            # empty margin is explainable instead of silently guessed.
            import sys as _sys
            print(
                f"[Gateway] whatIf {action} {abs(quantity)} "
                f"{getattr(c, 'symbol', '?')} ({order.orderType}"
                f"{('@'+str(round(px,2))) if px > 0 else ''}): "
            ...
            return {
                ...
                "maint_margin_after": maint_after,
                "equity_with_loan_after": _f(getattr(state, "equityWithLoanAfter", None)),
                "commission": _f(getattr(state, "commission", None)),
            }
        except Exception as e:
            # Non-fatal: whatIf is a best-effort authoritative overlay on
            # top of the offline ShortPolicy estimate. Never let it break
            # anything — the caller keeps the offline number.
            print(f"[Gateway] whatIf margin preview failed: {type(e).__name__}: {e}")
            return None

    async def _get_short_fee_rate(self, contract) -> Optional[float]:
        """Live annualised stock-borrow fee for `contract`, as a FRACTION
        (0.0025 = 25 bps/yr), from IBKR's FEE_RATE historical feed.

        This replaces the hardcoded `default_borrow_rate_annual` (25 bps)
        placeholder in RegTEquityShort. IBKR's `whatToShow="FEE_RATE"`
        returns the current annualised borrow fee in PERCENT (e.g. a bar
        close of 0.30 means 0.30%/yr), so we divide by 100 to hand the
        ShortPolicy a fraction.

        Best-effort: the short-borrow/SLB feed needs the right market-data
        entitlement and is not available on every account. Returns None on
        any failure — the caller keeps the offline placeholder rate. As the
        operator's own source-of-truth note says, the fee rate is most
        reliably confirmed post-fill from the account's interest/fee
        accrual; this is the live pre-trade best estimate.
        """
        if self._ib is None or not self.connected:
            return None
        try:
            ...
            if bars:
                rate_pct = float(bars[-1].close)
                # Guard against IBKR's -1 / NaN "no data" sentinels.
                if rate_pct >= 0:
                    return rate_pct / 100.0
        except Exception as e:
            print(f"[Gateway] FEE_RATE feed unavailable "
                  f"(keeping offline estimate): {type(e).__name__}: {e}")
        return None

    async def get_shortable_info(
        self,
        contract=None,
        timeout_s: float = 2.5,
    ) -> Optional[dict]:
        """LIVE shortability + borrow cost for a short — the authoritative
        replacement for the hardcoded `hard_to_borrow` bool and 25-bps
        borrow-rate placeholder in the ShortPolicy.

        Mechanism (per the operator's source-of-truth table):
          * Availability + HTB — `reqMktData(genericTickList="236")`
            populates `ticker.shortableShares` (# shares IBKR can locate)
            and `ticker.shortable` (a code). This is a live check, not a
            static flag.
          * Borrow fee rate — the FEE_RATE historical feed (see
            `_get_short_fee_rate`). Entitlement-gated; None when absent.

        Runs against the live IBKR connection even in paper mode (paper
        shares the linked live account's market-data entitlements, so the
        SLB/short feeds work when entitled). Returns None only when
        disconnected or the feed is unavailable/unentitled.

        Returns a dict (or None on disconnected / feed failure):
            shortable_shares : float | None  — # shares locatable now
            shortable_code   : float | None  — IBKR's raw shortability code
            available        : bool           — can we short it at all?
            hard_to_borrow   : bool           — elevated borrow / thin supply?
            fee_rate_annual  : float | None   — borrow fee as a FRACTION
        Only meaningful for instruments you actually borrow (equity); the
        engine gates the call on `spec.short.requires_locate`.
        """
        if self._ib is None or not self.connected:
            return None
        ...
        def _clean(x):
            # ib_async leaves unpopulated float ticks as NaN.
            if x is None:
                return None
            try:
                xf = float(x)
                ...
                return None
            return None if _math.isnan(xf) else xf
        ...
        try:
            ...
            if c is None:
                c = await self._get_contract()
            # 236 = "Shortable". Streaming (snapshot=False) so shortableShares
            # keeps updating; we poll the loop briefly for the first value.
            ticker = self._ib.reqMktData(c, genericTickList="236", snapshot=False)
            import time as _time
            deadline = _time.monotonic() + timeout_s
            while _time.monotonic() < deadline:
                if _clean(getattr(ticker, "shortableShares", None)) is not None or \
                   _clean(getattr(ticker, "shortable", None)) is not None:
                    break
                await asyncio.sleep(0.1)

            shares = _clean(getattr(ticker, "shortableShares", None))
            code = _clean(getattr(ticker, "shortable", None))
        except Exception as e:
            print(f"[Gateway] shortable feed failed: {type(e).__name__}: {e}")
            shares, code = None, None
        finally:
            # Always release the ad-hoc subscription so we don't leak a
            # market-data line (IBKR caps concurrent lines per account).
            if ticker is not None and c is not None:
                try:
                    self._ib.cancelMktData(c)
                except Exception:
                    pass

        fee_rate = await self._get_short_fee_rate(c) if c is not None else None

        # Availability: IBKR's `shortable` code is >=2.5 when ≥1000 shares
        # are readily available, >=2.0 when locatable on request, <2.0 /
        # unset when not shortable. Prefer the concrete share count.
        #
        # CRITICAL: distinguish "no data" from "not shortable". The tick-236
        # feed frequently DOESN'T populate — paper accounts, a missing
        # short-availability market-data entitlement, or the 2.5s poll timing
        # out — leaving both `shares` and `code` None. Treating that absence
        # as `available=False` FALSELY blocked shortable names (e.g. IBM)
        # that IBKR itself lets you short manually. So:
        #   • real data says available   → True
        #   • NO data at all (both None) → None  (UNKNOWN → gate fails OPEN)
        #   • data present but negative  → False (genuinely not shortable → block)
        if shares is not None and shares >= 1000:
            available = True
        elif code is not None and code >= 2.0:
            available = True
        elif shares is None and code is None:
            available = None  # unknown — feed never populated; do NOT block
        else:
            available = False  # concrete count <1000 or code <2.0 → not shortable

        # Hard-to-borrow: prefer the live fee — >100 bps/yr is the
        # conventional HTB line. Otherwise infer from thin supply.
        if fee_rate is not None:
            hard_to_borrow = fee_rate > 0.01
        elif shares is not None:
            hard_to_borrow = shares < 100_000
        else:
