...
─── 算  Algorithm · the hot-path gate + its caches ────────────────────────
 ...
 19: ── CACHE refresh · PortfolioReader._maybe_refresh() ──  lazy, behind TTL
     ...
     │                  + live.pending_notional     ▷ working-but-unfilled BUY counts
     │      ▷ both terms additive — sees a pending BUY's capital from submission
...
─── 注  Notes · invariants ───────────────────────────────────────────────
   ...
   • Pending counts as exposure.  PortfolioReader books pending_notional so two bots
     can't both pass while their combined fills breach the cap (the $35k>$25k bug).
