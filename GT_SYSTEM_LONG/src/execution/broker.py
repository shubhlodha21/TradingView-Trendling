...
class Gateway:
    ...
    __slots__ = (
        ...
        '_fill_ledger',
        # Fencing token for the ledger-freshness gate (EURUSD naked-short
        # 2026-07-28). Monotonic counter bumped +1 on every successful
        # connect. The engine compares it against its own _ledger_epoch to
        # decide whether the ledger has been reconciled for THIS connection
        # before letting any actuator place/cancel orders. Lives here because
        # connect() is the single choke point every (re)connection — the
        # supervisor, the engine's active-reconnect, or a flap — must pass
        # through, so it advances even for reconnects no loop ever observed.
        'connection_epoch',
    )
