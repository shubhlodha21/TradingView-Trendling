...
class Config:
    ...
    # `max_position_value_usd`: cap on a single entry order's notional
    # (qty × price). At $100k a 100-share order can't enter above $1000.
    ...
    # Raised 2026-06-02: $25k → $50k; 2026-06-25: $50k → $100k; 2026-07-02: $100k → $200k per operator decision.
    # Lowered 2026-07-02: $200k → $150k per operator decision.
    ...
    # Asset routing — when True, resolve the CFD contract for `ticker` instead
    # of the equity/spot contract (NVDA share-CFD vs NVDA equity, etc.). Set via
    # --cfd or GT_CFD=1. Default False → equity/FX resolution unchanged.
    cfd: bool = False

    # Entry order type. Default False = the historical behaviour: rest a BUY
    # STP-LMT at `trigger_price` and let IBKR fire it when LTP rises to the
    ...
    # crossing and launches this bot in response (e.g. the RTH trendline
    ...
    #
    # The trade-off, stated plainly: a MARKET entry has no price ceiling. It
    # fills at whatever the book offers when it lands, which may be worse than
    # `trigger_price` if the market moved between the crossing and this bot
    # connecting. STP-LMT can miss the trade; MARKET cannot miss it but can pay
    # more for it. `trigger_price` is still required — the protective child
    # stop is sized from it until the parent's real fill price is known.
