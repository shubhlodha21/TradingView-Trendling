...
class Engine:
    ...
    __slots__ = (
        ...
        # (the RTH trendline runner) saw exactly one crossing and started this
        # bot for it; that entry has nothing left to wait for and goes in at
        # market. Every LATER entry — the re-entry after an exit, the
        ...
        # market would buy instantly at the depressed post-exit ask, which is
        # the exact bug the re-entry comment in `_on_gateway_fill` records.
        '_market_entry_used',
        ...
        '_reconciling',
        # Fencing-token peer of gateway.connection_epoch (EURUSD naked-short
        # 2026-07-28). Holds the connection epoch the ledger was last
        # reconciled against. Actuators may act only when this equals
        # gateway.connection_epoch. NOT persisted — re-inits to -1 every
    ...
    def __init__(
        ...
        # Session window controller: auto-PAUSE outside ETH (04:00-20:00
        # ET Mon-Fri). Set `_rth_only=False` to disable (e.g. for futures
        # or test runs that need to ignore the equity session).
        self._session_controller_task: Optional[asyncio.Task] = None
        self._rth_only: bool = True
    ...
    def _protective_stop_price(self, entry_price, stop_pct) -> float:
        """Compute the protective SELL stop price with **zero float drift**.

        The user's complaint: BUY parent shows exactly what they typed
        (1.17005 → 1.17005 — no math, no rounding). But the SELL child
        stop is computed as `entry × (1 - stop_pct)` — that's float
        arithmetic, which has tiny precision errors that can shift the
        snapped result by one tick at grid boundaries.
        ...
        most precise math possible. For EURUSD with runtime tick of
        0.00005, a 1.16401 × 0.999 calculation produces exactly
        1.16285 (= 23257 ticks × 0.00005) — not "1.16285 modulo
        some float drift."
            ...
            if self._audit:
                try:
                    ...
                    self._audit.log_order(
                        ...
                        position_at_time="LONG" if self._position_open else "FLAT",
                    )
                except Exception:
                    pass
        except Exception as e:
    ...
    def _on_order_status_change(self, engine_id: str, status: str, message: str) -> None:
        ...
        if pending and pending.get('order_id') == engine_id:
            was_protective_stop = (pending.get('side') == OrderSide.SELL)
        ...
        if status == 'Rejected':
            ...
            # we only care about BUY entries (the only kind we place).
            if isinstance(engine_id, str) and engine_id.startswith(f"ENTRY_{OrderSide.BUY.value}_"):
    ...
    def _on_gateway_fill(self, order_id: str, qty: int, price: float, exec_id: Optional[str] = None,
        ...
        if side_value == OrderSide.BUY.value:
            ...
            # Only set initial peak; don't lower it on subsequent partials
            # (the high tracker only moves UP via _track_high anyway).
            if not self._highest_price or self._highest_price < avg_price:
                self._highest_price = avg_price
            if self._active_stop_pct is None:
                # First fill of this cycle — freeze the pct.
                #
                # RESTART-SAFETY: if a bracket child is already resting at
                # the broker (adopted via reconcile when disk state was
                # lost), its stop encodes the pct the operator ACTUALLY
                # placed the cycle with — which may differ from the CLI
                # --stop-pct of whatever session we restarted into. Back
                # that pct out of the resting child rather than defaulting
                # to config.stop_loss_pct, otherwise a restart into the
                # default (1%) silently widens a tighter stop (0.25% child
                # at 162.43 → 1% retarget at 161.21). Same inference the
                # legacy-state path uses in _load_state.
                ...
                if child_stop and avg_price and avg_price > 0:
                    self._active_stop_pct = max(
                        0.0, round(1.0 - (float(child_stop) / float(avg_price)), 6),
                    )
                    self._log(
                        f"SL pct recovered from resting bracket child: "
            ...
            # Snapshot IBKR's reported daily-high at cycle entry. _track_high
            # uses this baseline to detect when feed.high RISES during the
            # cycle (which can only happen from in-cycle trades) and bump
            # _highest_price even if our TRADE-tick feed didn't see the
            # higher print. Set ONCE on the first partial.
            if is_first_partial:
                try:
                    self._feed_high_at_entry = float(
                        self._feed.high if self._feed and self._feed.high else 0.0
                    )
                except Exception:
            ...
            if self.risk and is_first_partial:
                self.risk.record_fill(avg_price, cum_qty, "BUY")

            self._save_state()  # Persist position
            self._log(
                f"FILLED BUY {qty} @ ${price:.2f} "
            ...
            if getattr(self, '_alerts', None) is not None:
                try:
                    self._alerts.notify_trade(
                        ...
                        slippage=(price - (order.signal_price or price)) if order and order.signal_price else 0.0,
                        state=self._state.value,
                        cycle_id=getattr(self, '_cycle_id', ''),
                    )
                except Exception as e:
                    # Notification never breaks fill processing.
                    print(f"[notify_trade FILLED BUY] {e}")
                ...
                try:
                    ...
                    if is_complete:
                        ...
                        if child is not None:
                            ...
                            self._log(
                                f"BRACKET CHILD PROMOTE: modifying {child_id} "
                        ...
                        # risk: if price drops to the child stop right
                        # now, broker fires SELL 1000 against our 600 →
                        # goes SHORT 400. IBKR does NOT auto-resize the
                        # child to match the parent's running fill.
                        #
        ...
        elif side_value == OrderSide.SELL.value:
            # ── Phantom-SELL guard ─────────────────────────────────────
            ...
            if not self._position_open or not self._entry_price:
                # A52 lifecycle log: phantom path — this is the smoking-gun
                # event. The broker just told us a SELL filled, but the
                # engine never recorded the matching BUY. Tag it so the
                # post-mortem grep correlates with the upstream BACKFILL/
                # SCAN entries from this same reconnect cycle.
                print(
                    f"[BRACKET_LIFECYCLE] PHANTOM_SELL  sym={self.config.ticker}  "
                    f"order_id={order_id}  qty={qty}  price={price}  "
                ...
                self._log(
                    f"[PHANTOM-SELL] REJECTED: SELL fill {order_id} "
                ...
                if self._audit:
                    try:
                        self._audit.log_order(
                            ...
                            signal_price=0.0,
                            reason=(
                                f"SELL fill arrived while engine FLAT — "
                ...
                if self._alerts:
                    try:
                        ...
                        self._alerts.raise_alert(
                            ...
                            message=(
                                f"PHANTOM SELL fill rejected on "
            ...
            if not is_complete:
                ...
                self._log(
                    f"SELL PARTIAL FILL: {qty} @ ${price:.2f} "
                ...
                if self._audit:
                    self._audit.log_order(
                        event="PARTIAL_FILL",
                        order_id=order_id, side="SELL", qty=qty,
            ...
            # If the protective SELL stop fired while the entry BUY still had
            # unfilled qty resting at the broker, cancel that remainder.
            # Otherwise we'd: sell N shares, then later the remaining BUY would
            # fill (if price came back up to its limit), leaving us LONG the
            # remainder with no protective stop. Senior-quant invariant: never
            # be long without an SL.
            try:
                ...
                entry_id = self._make_engine_id(f"ENTRY_{OrderSide.BUY.value}", self.config.quantity)
                pending_buy = self.registry.get(entry_id) if self.registry else None
                if pending_buy and pending_buy.filled_qty < pending_buy.qty and pending_buy.status not in (
                    OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.REJECTED
                ):
                    self._log(
                        f"[SAFETY] SELL stop fired while BUY {entry_id} still had "
                        f"{pending_buy.qty - pending_buy.filled_qty} unfilled shares — "
                        f"cancelling BUY remainder"
                    )
                    if getattr(self, '_running', False):
                        try:
                            import asyncio
                ...
                # Best-effort — don't break the SELL fill processing.
                self._log(f"[SAFETY] BUY-remainder cancel check failed: {e}")
            ...
            if self.risk:
                ...
                # tracked separately.
                self.risk.record_fill(price, qty, "SELL")
                if pnl < 0:
            ...
            # Per doc section 11: Store breakout level for re-entry
            breakout_level = self._highest_price
            self._previous_breakout_level = breakout_level

            self._log(
                f"FILLED SELL {qty} @ ${price:.2f} [{exit_reason}], "
                f"P&L: ${pnl:+.2f} (gross: ${gross_pnl:+.2f}, comm: ${commission:.2f}), "
                f"breakout=${breakout_level or 0:.2f}, total_pnl=${self._pnl:+.2f}"
            )
            ...
            if getattr(self, '_alerts', None) is not None:
                try:
                    slip = entry - price  # SELL: signal - fill (positive = bad)
                    self._alerts.notify_trade(
                        "FILLED",
                        symbol=self.config.ticker,
                        side="SELL",
                    ...
                    print(f"[notify_trade FILLED SELL] {e}")
            ...
            # Place re-entry STOP-LIMIT immediately at previous breakout level
            # so the order rests at the broker and only fills when LTP RISES
            # back to the breakout. Previously this was a plain LIMIT, which
            # filled instantly at the depressed post-exit ask (well below the
            # intended re-entry price). _on_gateway_fill is sync, so schedule.
    ...
    async def start(self):
        ...
        # adoption path (engine placed a BUY, broker filled, engine crashed
        # before persisting entry_price/quantity to disk — the intent is
        # the only paper trail showing the order was ours).
        ...
        if intent_snap:
            ...
            if side_val == OrderSide.BUY.value:
                saved_buy_intent = {
        ...
        if (not _skip_naked) and (not self.gateway.paper) and self._state not in (
            ...
            if broker_qty > 0:
                # Is there a resting SELL stop at the broker for this position?
                has_sell_stop = bool(
                    getattr(self, '_pending_stop', None)
                    and self._pending_stop.get('side') == OrderSide.SELL
                )

                # ── Adoption criteria — two acceptable shapes ────────────
                # (1) STANDARD: saved entry_price + saved quantity both set
                #     AND saved qty matches broker. The "clean restart of
                #     a known cycle" case.
                state_evidence_standard = (
                    self._entry_price and self._entry_price > 0
                    and self._quantity and self._quantity == broker_qty
                )
                ...
                state_evidence_lost_fill = bool(
                    saved_buy_intent
                    and saved_buy_intent['qty'] == broker_qty
                ...
                if state_evidence:
                    ...
                    if state_evidence_lost_fill:
                        ...
                        self._quantity = broker_qty
                        self._log(
                            f"!!! LOST-FILL RECOVERY: saved state had pending BUY "
                            f"{saved_buy_intent['qty']} {self.config.ticker} but no "
                            f"recorded fill — broker now reports {broker_qty} shares "
                            f"at avg ${broker_avg_cost:.2f}. The fill happened during "
                            f"the prior session but state wasn't persisted. "
                            f"Hydrating entry_price=${self._entry_price:.2f}, "
                            f"quantity={self._quantity}."
                        )
                    else:
                        self._log(
                            f"!!! ORPHAN POSITION DETECTED: broker has {broker_qty} "
                            f"{self.config.ticker} matching saved state "
        ...
        if self._state in (TradeState.IN_POSITION, TradeState.EXIT_POSITION):
            ...
            # BUY STOP-LIMIT placement — idempotent: if reconciliation
            # already attached a resting BUY stop-limit (_pending_stop is
            # set), the helper returns None without placing a second one.
            self._log(f"Placing BUY STOP_LIMIT @ {entry_label} ${trigger_price:.2f}")
    ...
    def _load_state(self) -> bool:
        ...
        # stop (0.25% child at 162.43 → 1% retarget at 161.21). When a
        # bracket child was restored, hydrate `_active_stop_pct` from the
        # persisted pct so the eventual parent fill honours it.
    ...
    async def _reconcile_open_orders(self) -> None:
        ...
        # Fencing token (EURUSD naked-short 2026-07-28): capture the
        # connection epoch we are about to reconcile against. Stamp
        # _ledger_epoch only on CLEAN completion (below) so a raised reconcile
        # leaves the gate closed — a half-built ledger is never marked
        # "trusted" for this connection. If the connection flaps mid-reconcile
        # gateway.connection_epoch advances past this value, so the stamp
        # won't match → _actuation_allowed() stays closed → another reconcile
        # is forced. Capturing at START (not end) is what makes that safe.
    ...
    def _actuation_allowed(self) -> bool:
        ...
        may act — it must DEFER. Deferring is fail-safe: the open position
        stays covered by the resting broker bracket in the meantime.
    ...
    async def _reconcile_open_orders_inner(self) -> None:
        ...
        for o in open_orders:
            ...
            if action == 'BUY' and otype_norm == 'STPLMT':
                engine_id = broker_ref if broker_ref else self._make_engine_id(
                    f"ENTRY_{OrderSide.BUY.value}", qty
                )
                side = OrderSide.BUY
            ...
            elif action == 'SELL' and otype_norm == 'STPLMT':
                engine_id = broker_ref if broker_ref else self._make_engine_id(
                    f"SL_{OrderSide.SELL.value}", qty
                )
                side = OrderSide.SELL
                otype = OrderType.STOP_LIMIT
                state_matches = self._state in (
                ...
                is_bracket_child = False
            elif action == 'SELL' and otype_norm == 'STP':
                # Bracket child — market on trigger. Prefer orderRef so we
                # recover the exact cycle_seq; otherwise reconstruct from
                # convention with the current seq (best-effort fallback
                # for legacy/manual orders).
                engine_id = broker_ref if broker_ref else self._make_engine_id(
                    f"BR_{OrderSide.SELL.value}", qty
                )
                side = OrderSide.SELL
            ...
            if side == OrderSide.SELL and otype in (OrderType.STOP, OrderType.STOP_LIMIT):
                ...
                if is_prior_session and not is_saved_bracket and not engine_holds:
                    self._log(
                        f"Reconcile: PRIOR-SESSION ORPHAN SELL {action} {otype_norm} "
                    ...
                    try:
                        ...
                        self._log(f"Reconcile: A82 orphan SELL cancel raised: {e}")
                    if self._audit:
                        try:
                            self._audit.log_order(
                                ...
                                reason=(
                                    f"A82: prior-session orphan SELL (session "
                                    f"{order_session} != current {self._session_id}), "
                                    f"engine FLAT — cancelled to prevent a naked short."
                                ),
                                context={
                                    "ticker": self.config.ticker,
                                    "broker_id": broker_id,
            ...
            #
            # Scenario: a previous strategy run on this ticker used
            # qty=100. That run left a SELL STP for 100 shares resting at
            # IBKR. The current run uses qty=30. On startup, reconcile
            # would (previously) adopt the 100-share SELL as our bracket
            # child. When the bracket parent fills and the SELL eventually
            # triggers, IBKR sells 100 shares against our 30-share
            # position → broker goes SHORT 70.
            #
            # The fix: when adopting a SELL, require its qty to match the
            # engine's expected position size. If it doesn't, the leg is a
            # stale orphan from a different config — cancel it at the
            # broker, audit + alert, and skip adoption. Don't register it
            # in our books at all.
            #
            # Expected position qty = self._quantity if we currently hold a
            # position, otherwise self.config.quantity (the size the
            # current cycle would enter at). The first applies after the
            # parent fills; the second applies before — both must match
            # whatever the engine would size a fresh SL to.
            if side == OrderSide.SELL and otype in (OrderType.STOP, OrderType.STOP_LIMIT):
                expected_qty = (
                    int(self._quantity)
                ...
                if int(qty) != expected_qty:
                    self._log(
                        f"Reconcile: REJECTING stale SELL {action} {otype_norm} "
                    ...
                    try:
                        ...
                        self._log(f"Reconcile: orphan SELL cancel raised: {e}")
                    if self._audit:
                        try:
                            self._audit.log_order(
                                event="STALE_SELL_REJECTED",
                                order_id=engine_id,
                                side="SELL",
                                qty=qty,
                                order_type=otype_norm,
                                ...
                                reason=(
                                    f"Stale SELL detected on reconcile. "
                                    f"broker_qty={qty}, engine_expected_qty={expected_qty}. "
                                    f"Cancelled at IBKR to prevent qty-mismatch short "
                                    f"(the 2026-06-02 PLTR class of bug)."
                                ),
                                state_at_time=self._state.value,
                                position_at_time=(
                                    "LONG" if self._position_open else "FLAT"
                                ),
                            )
                        except Exception:
                            pass
                    if self._alerts:
                        try:
                            ...
                            self._alerts.raise_alert(
                                ...
                                severity=AlertSeverity.HIGH,
                                message=(
                                    f"Reconcile rejected stale SELL on "
                                    f"{self.config.ticker}: broker has SELL for "
                                    ...
                                    f"shorted {qty - expected_qty} shares."
                                ),
                                context={
                                    "ticker": self.config.ticker,
                                    "broker_id": broker_id,
                                    ...
                                    "would_have_shorted": qty - expected_qty,
                                },
                                correlation_id=getattr(self, '_cycle_id', ''),
                            )
                        except Exception:
        ...
        if (
            ps
            and ps.get('side') == OrderSide.BUY
            and isinstance(ps.get('order_id'), str)
            and ps['order_id'].startswith('ENTRY_BUY_')
                    ...
                    if (o.get('action') == 'SELL'
                        ...
                        child_engine_id = (
                            ...
                            or self._make_engine_id(
                                f"BR_{OrderSide.SELL.value}", expected_qty
                            )
                        )
                        # Mirror the shape used by place_bracket_buy_stop_market.
                        self._bracket_child = {
        ...
        if (
            ...
            # ── RESTART-SAFETY: recover the frozen pct from the resting
            #    child BEFORE computing expected_stop ─────────────────────
            # On a broker-only recovery (parent filled during downtime, disk
            # state lost), reconcile recovers `position_open`/`entry_price`
            # for the PARENT from the broker position — but the CHILD's
            # frozen SL pct (`_active_stop_pct`) has no other source and is
            # still None here. `_effective_stop_pct()` would then fall back
            # to config.stop_loss_pct (e.g. 1%), making `expected_stop` the
            # 1% level (161.21) — which DIFFERS from the operator's real
            # resting child (0.25% at 162.43), so the block below would
            # ACTIVELY re-modify the good stop to the wrong one. Back the
            # pct out of the resting child's own stop instead: expected_stop
            # then equals the child (no-op), or differs only by the tiny
            # trigger→fill refinement this recovery legitimately handles.
            if self._active_stop_pct is None and current_stop > 0:
                self._active_stop_pct = max(
                    0.0, round(1.0 - (current_stop / self._entry_price), 6),
                )
                self._log(
                    f"Reconcile: recovered frozen SL pct "
    ...
    async def _gap_fill_highest_price(self) -> None:
        ...
        # Max high across the window. Filter out zeros (incomplete bars).
        try:
            gap_high = max(b.high for b in bars if getattr(b, 'high', 0) > 0)
        except ValueError:
            return  # empty after filter

        prior = self._highest_price or 0.0
        if gap_high > prior:
            self._log(
                f"[GAP-FILL] _highest_price ${prior:.2f} → ${gap_high:.2f} "
                f"(scanned {len(bars)} {bar_size} bars across {gap_seconds:.0f}s "
                f"disconnect; protected re-entry breakout level from "
                f"premature trigger)"
            )
            self._highest_price = gap_high
            # Persist now so the eventual SELL replay sees the bumped value
            # even if reconcile spans multiple awaits.
            self._save_state()
            ...
            if self._alerts:
                ...
                self._alerts.raise_alert(
                    ...
                    message=(
                        f"_highest_price bumped from ${prior:.2f} to ${gap_high:.2f} "
                        f"during reconnect (disconnect was {gap_seconds:.0f}s); "
                        f"re-entry breakout level is now accurate to the real peak."
                    ),
                    context={
                        "ticker": self.config.ticker,
                        "prior_high": prior,
    ...
    async def _gap_fill_peak_window(self, start_time, end_time) -> Optional[float]:
        ...
        try:
            ...
            return None  # all bars had high == 0

        prior = self._highest_price or 0.0
        if gap_high > prior:
            self._log(
                f"[RECONCILE GAP-FILL] inter-fill peak ${prior:.2f} → ${gap_high:.2f} "
                f"(scanned {len(bars)} {bar_size} bars across {gap_seconds:.0f}s "
                f"between BUY and SELL fills; re-entry breakout level corrected "
                f"to the real intra-cycle high)"
            )
            self._highest_price = gap_high
            # Persist immediately so the SELL replay (next call in the
            ...
            if self._alerts:
                ...
                self._alerts.raise_alert(
                    ...
                    severity=AlertSeverity.LOW,
                    message=(
                        f"Inter-fill peak recovered: ${prior:.2f} → ${gap_high:.2f}. "
                        f"Both BUY and SELL filled offline; re-entry breakout level "
                        f"is now anchored to the real intra-cycle high (was about to "
    ...
    async def _recompute_realized_pnl_from_broker(self) -> None:
        ...
        # Walk: FIFO inventory of open BUYs. SELLs match earliest BUYs.
        from collections import deque
        inventory: deque = deque()  # each: {'qty': float, 'price': float, 'comm': float}
        realized = 0.0
        matched_trips = 0
        ...
        for f in ours:
            ...
            if ib_side == 'BOT':
                ...
                remaining = shares
                sell_comm_per_share = (comm / shares) if shares > 0 else 0.0
                while remaining > 0 and inventory:
                    ...
                    # Round-trip PnL: (sell - buy) * units - (proportional commission)
                    trip = (price - lot['price']) * used
                    ...
                    try:
                        ...
                        pass
                    trip -= buy_comm_per_share * used
                    trip -= sell_comm_per_share * used
                    realized += trip
                    matched_trips += 1
                    lot['qty'] -= used
                    lot['comm'] -= buy_comm_per_share * used
                    remaining -= used
                    if lot['qty'] <= 1e-9:
                        inventory.popleft()
                # If SELL has unmatched qty (no inventory), it's a SHORT —
                # not supported by this strategy; just log and skip the
                # remainder. The position-reconcile path will catch the
                # divergence separately.
                if remaining > 0:
                    self._log(
                        f"[PNL-RECOMPUTE] SELL execution has {remaining} "
                        f"units with no matching BUY in today's history "
                        f"— likely a position carried from yesterday or "
                        f"a manual TWS trade. Skipping for PnL math."
                    )

        # Refresh `_pending_buy_commission` from broker truth if a
        # position is currently open. Without this, the next SELL fill
        # would close the round-trip using whatever stale value the
        # state file had for `_pending_buy_commission` — which was
        # written by the OLD modeled-estimate code path ($92 instead
        # of $2 for EURUSD). Inventory's remaining BUY lot(s) carry
        # the true broker commissions accumulated above.
    ...
    async def _reconcile_missed_fills(self) -> None:
        ...
        for fill_time, fill in candidates:
            try:
                ...
                if not engine_id:
                    if side_str == 'BUY':
                        engine_id = self._make_engine_id(f"ENTRY_{OrderSide.BUY.value}", qty)
                    else:
                        engine_id = self._make_engine_id(f"SL_{OrderSide.SELL.value}", qty)
    ...
    async def _broker_qty_for_symbol(self) -> Optional[int]:
        """Return the broker's current share count for `self.config.ticker`.

        Returns:
            int >= 0   — broker holding (0 if flat)
    ...
    async def _preflight_sell_allowed(
        ...
        """Pre-flight guard: confirm the broker actually has `qty_to_sell`
        shares before we submit a SELL order.

        If broker has fewer shares than we're about to sell, the SELL
        would either reduce-and-short (broker_qty < qty_to_sell) or
        purely short (broker_qty == 0). Refuse the placement, fold the
        engine to FLAT to match broker truth, and fire CRITICAL alert.

        Returns:
            True  — safe to proceed (broker has >= qty_to_sell shares
                    OR query failed and we're letting it through to
                    avoid blocking trading on a network glitch).
            False — refuse the SELL. Engine has been folded to FLAT.

        Paper mode: always returns True (paper sim handles its own
        positions and can't short us in IBKR).
        """
        if self.gateway.paper:
            return True

        engine_qty = self._quantity if self._position_open else 0
        broker_qty = await self._broker_qty_for_symbol()

        if broker_qty is None:
            # Couldn't query — log and allow. Don't block legitimate
            # trading on a single API hiccup. The next reconcile tick
            # (or a successful pre-flight on the next SELL attempt)
            # will catch any real divergence.
            self._log(
                f"[PRE-FLIGHT/{context}] broker qty query failed — "
                f"allowing SELL (will recheck on next attempt)"
            )
            return True

        if broker_qty >= qty_to_sell:
            # Broker has enough shares — safe to place this SELL.
            return True
        ...
        if is_fx_cash and broker_qty == 0:
            self._log(
                f"[PRE-FLIGHT/{context}] FX_CASH_LEDGER_GUARD: positions() "
                f"returns 0 for {self.config.ticker} (cash-ledger quirk). "
                f"Engine has LONG {engine_qty} on record — allowing SELL. "
                f"Net account-currency exposure is the source of truth, "
                f"not positions(); operator can verify via "
                f"inspect_broker.py if needed."
            )
            return True

        # Broker has fewer shares than we're about to sell. Refuse.
        #
        # A53 (2026-06-09): we used to also fold engine state to FLAT here.
        # That was dangerous in the WRONG-direction failure mode: a transient
        # positions() glitch (lag, FX cash-ledger quirk, momentary cache miss)
        # could read broker_qty=0 on a real LONG position, and the fold would
        # then cancel the bracket child / protective SL — stripping the
        # protection off a live position. The pre-flight refusal alone is
        # the actual protection against shorting; folding state on top is
        # redundant and risk-positive. Operator handles reconciliation
        # manually when this fires.
        self._log(
            f"[PRE-FLIGHT/{context}] REFUSING SELL {qty_to_sell} {self.config.ticker} — "
            f"broker only has {broker_qty} shares (engine thought {engine_qty}). "
            f"Placing this order would short {qty_to_sell - max(broker_qty, 0)} shares. "
        ...
        if self._alerts:
            try:
                ...
                self._alerts.raise_alert(
                    ...
                    severity=AlertSeverity.CRITICAL,
                    message=(
                        f"Pre-flight blocked a SELL that would have shorted "
                        f"{qty_to_sell - max(broker_qty, 0)} {self.config.ticker} "
                        f"(context={context}, broker_qty={broker_qty}, "
                        f"engine_qty={engine_qty}, qty_to_sell={qty_to_sell}). "
        ...
        if self._audit:
            try:
                self._audit.log_order(
                    ...
                    order_type="GUARD",
                    reason=(
                        f"Pre-flight refused SELL: broker={broker_qty}, "
                        f"engine={engine_qty}, qty_to_sell={qty_to_sell}, "
                        f"context={context}"
                    ),
                    state_at_time=self._state.value,
                    position_at_time="LONG" if self._position_open else "FLAT",
    ...
    async def _reconcile_position_state(self) -> None:
        ...
        if broker_qty != engine_qty:
            ...
            if ledger_backed and broker_qty > 0:
                # Ledger holds a position the engine is blind to (or sized
                # wrong). Adopt it as IN_POSITION so the engine stops arming
                # fresh entries on top, and re-arm a CORRECTLY-sized stop.
                old = engine_qty
                self._entry_price = (
                ...
                # protective SELL sized to the adopted quantity.
                self._pending_stop = None
                self._bracket_child = None
                self._save_state()
                self._log(
                    f"[FL8] ADOPTED ledger position: {self.config.ticker} "
                    f"engine {old} → {self._quantity} (entry≈${self._entry_price}); "
                    f"stale bracket dropped, protective stop will re-arm to size."
                )
                try:
                ...
                if self._alerts:
                    ...
                    self._alerts.raise_alert(
                        ...
                        severity=AlertSeverity.HIGH,
                        message=(f"FL8 self-heal: adopted {self._quantity} "
                                 f"{self.config.ticker} from ledger (engine was "
                                 f"{old}); protective stop re-armed."),
                        context={"ticker": self.config.ticker,
                                 "adopted_qty": self._quantity, "was": old},
                        correlation_id=self._cycle_id,
                    )
            elif ledger_backed and broker_qty == 0 and engine_qty != 0:
                # Ledger says FLAT; engine thought it held a position → the
                # CLOSE was missed. Safe to fold to FLAT because the trusted
                ...
                self._log(
                    ...
                    f"{self.config.ticker}; folded engine to FLAT (close was "
                    f"missed). Re-entry will arm normally.")
            elif ledger_backed and broker_qty < 0:
                # Naked short — long-only strategy. Adopt the knowledge but do
                # NOT arm (covering = BUY; left to teardown/operator). Alert.
                self._log(
                    f"[FL8] LEDGER SHORT {broker_qty} {self.config.ticker} — "
                    f"naked short detected; entries stay blocked, alerting.")
                if self._alerts:
                    from src.infra.alerts import AlertSeverity
                    self._alerts.raise_alert(
                        code="CUSTOM_LEDGER_SHORT",
                        severity=AlertSeverity.CRITICAL,
                        message=(f"FL8: ledger shows NAKED SHORT {broker_qty} "
                                 f"{self.config.ticker} — needs flatten (BUY to cover)."),
                        context={"ticker": self.config.ticker, "short_qty": broker_qty},
                        correlation_id=self._cycle_id,
                    )
    ...
    async def _session_controller_loop(self) -> None:
        ...
        while self._running:
            ...
            if in_session and last_entries_allowed and not entries_ok:
                ...
                # KEEP the resting BUY entry bracket through the close so it
                # persists overnight (the reconcile flow). Set GT_ENTRY_CUTOFF=1
                # to restore the old behaviour: cancel the pending BUY in the
                # last ENTRY_CUTOFF_BUFFER_MIN minutes before close. Either way
                # the protective SELL stops and the session PAUSE below are
                ...
                if not _do_cutoff_cancel:
                    self._log(
                        ...
                        "(pending BUY entry left resting through the close; set "
                        "GT_ENTRY_CUTOFF=1 to cancel)"
                    )
                elif pending and (pending.get('side') == 'BUY' or pending.get('side') == OrderSide.BUY):
                    pending_id = pending.get('order_id') or pending.get('id') or ''
                    self._log(
                        f"[SESSION] Entry cutoff reached — cancelling pending entry "
                        f"BUY order {pending_id} (no new entries within last "
                    ...
                    self._log(
                        f"[SESSION] Entry cutoff reached — no pending BUY to cancel "
                        f"(state={self._state.value})"
                    )
        ...
        while self._running:
            try:
                ...
                if (getattr(self, '_entry_placing', False)
                        ...
                        # Fencing-token gate (EURUSD naked-short 2026-07-28):
                        # also skip the orphan sweep whenever the ledger is
                        # not reconciled for the current connection epoch —
                        # a stale _bracket_child / _pending_stop would let it
                        # cancel a leg it is about to re-adopt. Superset of the
                        # _reconciling check above; never skips less.
    ...
    async def _health_check_loop(self) -> None:
        ...
        while self._running:
            ...
            # Fencing-token gate (EURUSD naked-short 2026-07-28). Probe 1
            # (NAKED_POSITION -> arm SELL) and Probe 2 (ENTRY_ORDER_MISSING ->
            # place BUY) both READ ledger position state to decide whether to
            # actuate. Do NOT let them act until the ledger has been
            # reconciled against broker executions for the CURRENT connection
            # epoch. Probe 0 above is a positions()-based check that is blind
            # for FX (cash-ledger quirk), so the epoch is the real gate: it
            # was the ~0.5s window between reconnect and the executions-replay
            # fold that armed the naked short. Deferring is fail-safe — the
            # position stays covered by the resting broker bracket; we re-probe
            # next tick, and _after_reconnect's reconcile opens the gate.
            ...
            if has_sell_sl and self._pending_stop and self._pending_stop.get('side') == OrderSide.SELL:
                engine_stop = float(self._pending_stop.get('stop_price') or 0)
                # Find the resting SELL STP that matches OUR engine_id
                # (skip any unrelated SELL stops the user may have placed
                ...
                if engine_stop > 0 and broker_stop_for_ours is not None:
                    ...
                    if delta > tol:
                        ...
                        msg = (
                            ...
                            f"{engine_stop:.6f}, broker has resting SELL STP at "
            ...
            if self._position_open and self._entry_price and not has_sell_sl:
                # CRITICAL: Don't re-arm if there's a working BUY entry still
                # filling. Production hit 2026-05-27 (NVDA): BUY partial of
                # 600/1000 set `_quantity=600`, then health-check fired ~140ms
                # later, saw "position open, no SL", and placed an SL sized
                # to 600. Remaining 400 BUY shares filled afterward → broker
                # held 1000 LONG but SL covered only 600. The next 400 sat
                # NAKED until cycle close.
                #
                # The right behavior is to wait: if a BUY entry is still
                # working at the broker, the proactive `is_complete=True`
                # branch in `_on_gateway_fill` will fire the SL at the
                # right qty as soon as the BUY fully fills. Health check
                # re-arm is a SAFETY NET for the case where that path
                # failed — but only AFTER the BUY is truly done.
                buy_entry_still_working = False
                try:
                    entry_id = self._make_engine_id(f"ENTRY_{OrderSide.BUY.value}", self.config.quantity)
                    ...
                    if entry_order is not None and entry_order.filled_qty < entry_order.qty:
                        ...
                        if entry_order.status not in (
                            ...
                            buy_entry_still_working = True
                    # Also belt-and-suspenders: check the broker's open-orders
                    # for any BUY STP LMT that isn't ours by id but is on the
                    # same ticker.
                    if not buy_entry_still_working and has_buy_sl:
                        buy_entry_still_working = True
                except Exception as e:
                    self._log(f"[HEALTH] BUY-in-flight check failed (treating as safe to re-arm): {e}")

                if buy_entry_still_working:
                    self._log(
                        f"[HEALTH] Position open without SL, but BUY entry still "
                        f"filling — DEFERRING re-arm. The BUY's `is_complete=True` "
                        f"branch will place the SL at full qty when it completes."
                    )
                ...
                if not last_alerted_naked and self._alerts:
                    self._alerts.raise_alert(
                        code="NAKED_POSITION",
                        severity=AlertSeverity.CRITICAL,
                        message=f"Position open ({self._quantity} {self.config.ticker} @ ${self._entry_price:.2f}) but NO protective SELL stop-limit at broker.",
            ...
            # ---- Probe 2: MONITORING/WAITING_REENTRY (not paused) ⇒ BUY entry resting
            should_have_entry = (
                self._state in (TradeState.MONITORING, TradeState.WAITING_REENTRY)
                and not self._paused
                and not self._position_open
            )
            if should_have_entry and not has_buy_sl:
                ...
                if rejected_recently:
                    ...
                    if not last_alerted_no_entry and self._alerts:
                        self._alerts.raise_alert(
                            ...
                            severity=AlertSeverity.HIGH,
                            message=f"Engine in {self._state.value} but NO BUY entry order resting at broker.",
                    ...
                    try:
                        if getattr(self, '_audit', None):
                            ...
                            self._audit.log_order(
                                event="HEALTH_REARM_DIAG", order_id="", side="BUY",
                                qty=0, order_type="",
                                reason=(f"health re-arm: has_buy_sl={has_buy_sl} "
    ...
    async def _force_market_exit(self, reason: str) -> None:
        ...
        # Step 3: submit MARKET sell. order_id is unique-per-cycle so the
        # registry doesn't collide with a prior SL_SELL record. Use the
        # _n{cycle_seq} suffix; the previous _cycle_id-based naming had
        # the same staleness problem since _cycle_id never bumps mid-run.
        market_id = self._make_engine_id(f"FORCE_{OrderSide.SELL.value}", qty)
        order = OrderRecord(
            order_id=market_id,
            symbol=self.config.ticker,
            side=OrderSide.SELL,
        ...
        self._log(f"[{reason}] Submitting MARKET SELL {qty} {self.config.ticker} (LTP~${ltp_for_log:.2f})")
        ...
        try:
            ...
            self._log(f"[{reason}] MARKET SELL failed: {e}")
    ...
    def get_summary(self) -> dict:
        ...
        return {
            "state": self._state.value,
            "position": "LONG" if self._position_open else "FLAT",
            "entry_price": self._entry_price,
            "quantity": self._quantity,
            "unrealized_pnl": ((self._prev_ltp - self._entry_price) * self._quantity) if self._position_open and self._entry_price else 0,
    ...
    async def _process_tick(self, tick: "Tick"):
        ...
        # Track position. The SELL-side price (bid for FX, last for
        # equity) is what we'd actually receive on the SL fire, so
        # _track_position and _check_stop_limit both evaluate against
        # sell_px in the IN_POSITION branch.
        if self._state == TradeState.IN_POSITION:
            await self._track_position(sell_px, tick)
            if hasattr(self, '_pending_stop') and self._pending_stop:
                await self._check_stop_limit(sell_px)

        # SELL submitted but not yet filled — keep tracking the peak so
        # the re-entry breakout level reflects the full holding period.
        # The stop check intentionally does NOT run here: the exit is
        # already in flight at the broker and re-running it would place
        # duplicate stop-limits.
        elif self._state == TradeState.EXIT_POSITION:
            self._track_high(ref_px)

        # Pre-position: BUY trigger. Use the BUY-side price (ask for
        # FX, last for equity) — entry fires when ask >= trigger
        # because that's what we'd actually PAY.
    ...
    def _log_state_and_pnl(self, ltp: float):
        ...
        if self._position_open and self._entry_price:
            unrealized = (ltp - self._entry_price) * self._quantity
    ...
    def _track_high(self, ltp: float):
        """Update _highest_price if a new peak is detected. Per doc section 9.
        ...
        """
        candidate = ltp
        # Augment with IBKR's aggregated daily-high if it has risen above
        # the per-cycle baseline. Guard on _position_open so this never
        # leaks pre-cycle highs into _highest_price (the baseline being
        # whatever feed.high was at the moment of entry).
        if (
            self._position_open
            and self._feed is not None
        ):
            try:
                fh = float(self._feed.high or 0)
                if fh > (self._feed_high_at_entry or 0) and fh > candidate:
                    candidate = fh
            except Exception:
                # Defensive — never let high-tracking math break the engine.
                pass
        if candidate > (self._highest_price or 0):
            self._highest_price = candidate
            self._log(f"New high: ${candidate:.2f}")
            self._save_state()
    ...
    async def _track_position(self, ltp: float, tick: "Tick"):
        ...
        if (
            ...
            and not getattr(self, '_pending_stop', None)
            # Fencing-token gate (EURUSD naked-short 2026-07-28): this
            # reactive fallback fires on the engine's BELIEF of IN_POSITION.
            # After a reconnect, before reconcile folds a phantom position,
            # that belief can be stale — firing _exit here would SELL into a
            # flat book (naked short via the tick path, the parallel door to
            # the health re-arm). Only act once the ledger is reconciled for
            # the current connection epoch. Fail-safe: defer until then.
    ...
    async def _check_stop_limit(self, ltp: float):
        ...
        Direction:
            SELL stop-limit (protective): triggers when ltp <= stop_price.
            BUY  stop-limit (entry):      triggers when ltp >= stop_price.
        """
        if not self.gateway.paper:
            return
        pending = getattr(self, '_pending_stop', None)
    ...
    async def _place_entry_stop_limit_inner(self, trigger_price: float) -> Optional[str]:
        """Place a BUY stop-limit at the broker to enter on breakout UP.

        Mirror of `_place_protective_stop` for entries. The strategy wants to
        buy only when LTP **rises** to `trigger_price`; a plain BUY LIMIT would
        fill immediately at any ask ≤ trigger (i.e. fill near the current
        market, often well below the intended breakout). A BUY STOP-LIMIT
        rests at the broker and activates only when LTP rises to the trigger.

        Geometry (mirror of SELL exit):
            stop_price  = trigger_price             (activates on breakout up)
            limit_price = trigger_price + sl_offset (ceiling we'll pay; IBKR
                          requires lmtPrice >= stopPrice for a BUY stop-limit)
        ...
        if self.risk:
            ...
            if not result.allowed:
                ...
                rej_id = f"RISK_BLOCK_{OrderSide.BUY.value}_{qty}_{self.config.ticker}_{self._cycle_id}"
                if self._audit:
                    self._audit.log_order(
                        event="REJECTED",
                        order_id=rej_id,
        ...
        # MARKET entry (--market): the crossing was detected upstream, so there
        ...
        # *about*, and the child's protective stop is sized from the trigger —
        # but neither is sent to IBKR for the parent leg.
        #
        # ONE-SHOT, and the guard matters more than the feature: this function
        # also places the re-entry after an exit (`_on_gateway_fill`) and the
        # session-open entry. Those target a breakout level price has NOT
        # reached — firing them at market would buy instantly at the depressed
        # post-exit ask, the precise regression the re-entry comment records.
        # Only the first entry of the process, the one the upstream signal
        # launched us for, may go in at market.
        entry_market = (
        ...
        order_id = self._make_engine_id(f"ENTRY_{OrderSide.BUY.value}", qty)
        ...
        # Submit BOTH parent BUY STP-LMT AND child SELL STP (market on
        # trigger) atomically. The child rests at the broker the instant
        # the bracket is accepted — so even if this engine instance dies
        # before the parent fills, the child is already armed at IBKR.
        # This is the senior-prescribed fix for the 2026-05-27 NVDA
        # incident where the trigger+limit got skipped in one tick.
        #
        # Child's initial stop_price is computed from `trigger_price`
        # (the BUY's intended fire level) — best estimate before we
        # know actual fill VWAP. After parent fills, `_on_gateway_fill`
        # BUY branch calls `gateway.modify_stop_trigger` to retarget the
        ...
        child_order_id = self._make_engine_id(f"BR_{OrderSide.SELL.value}", qty)

        self._log(
            (f"ENTRY BRACKET: parent BUY MARKET (--market; crossing already "
             f"detected upstream at ${stop_price:.2f}, no resting trigger, "
             f"NO price ceiling)"
             if entry_market else
             f"ENTRY BRACKET: parent BUY STP-LMT trigger=${stop_price:.2f} "
             f"limit=${limit_price:.2f}")
            + f"; child SELL STP-MKT trigger=${child_initial_stop:.2f} "
        ...
        try:
            bracket_ids = await self.gateway.place_bracket_buy_stop_market(
                qty=qty,
        ...
        #   - _pending_stop = parent BUY (as before, so existing
        #     idempotency / reconcile logic keeps working)
        #   - _bracket_child = child SELL (new; promoted to _pending_stop
        #     when parent fills)
        self._pending_stop = {
            'order_id': order_id,
            'qty': qty,
        ...
        if self._audit:
            self._audit.log_order(
                ...
                stop_price=child_initial_stop,
                signal_price=child_initial_stop,
                reason=f"bracket child of {order_id}; initial trigger from BUY trigger",
                state_at_time=self._state.value,
                position_at_time="FLAT",
            )
        ...
        # user stop (0.25% child at 162.43 → 1% retarget at 161.21).
        # _save_state already serializes both fields (stop_loss_pct +
        # bracket_child); _load_state hydrates them on the next start.
        self._save_state()
    ...
    async def _place_protective_stop_inner(self, reason: str) -> Optional[str]:
        ...
        if not self.gateway.paper:
            try:
                ...
                if existing_sells:
                    ...
                    self._log(
                        f"[{reason}] DUPLICATE-SELL GUARD: a SELL is "
                    ...
                    if self._audit:
                        try:
                            self._audit.log_order(
                                ...
                                reason=(
                                    f"Existing SELL resting at broker — "
                                    f"refused to place duplicate (context="
                                    f"{reason}). Adopted instead. Closes the "
                                    f"ZM 2026-06-01 double-shorting class of bug."
                                ),
                                state_at_time=self._state.value,
                                position_at_time=(
                                    "LONG" if self._position_open else "FLAT"
                    ...
                    if self._alerts:
                        try:
                            ...
                            self._alerts.raise_alert(
                                ...
                                severity=AlertSeverity.MEDIUM,
                                message=(
                                    f"Duplicate SELL refused for "
                                    f"{self.config.ticker}: existing SELL "
        ...
        # ── Gap-down guard ───────────────────────────────────────────
        # Senior-approved escape hatch retained from the legacy STP-LMT
        # path: when current LTP has already crossed below our intended
        # stop_price (gap-down on open, or a market move during
        # downtime), a STP order resting at IBKR triggers fine but the
        # next print after trigger could be FAR below the level we
        # intended — exposing us to extra slippage. The MARKET fallback
        # path sells AT current LTP, which is at least observable and
        # exits the position predictably. STP-MARKET wouldn't fix this
        # case (the fill price is whatever IBKR's next print is, which
        # in a fast gap-down move can be deep below our trigger).
        ltp_now = await self._current_ltp_best_effort()
        if ltp_now is not None and ltp_now > 0 and ltp_now <= stop_price:
            self._log(
                f"GAP-DOWN DETECTED: LTP=${ltp_now:.2f} ≤ intended stop trigger "
                f"${stop_price:.2f} — escalating to immediate MARKET SELL "
        ...
        order_id = self._make_engine_id(f"SL_{OrderSide.SELL.value}", qty)
        ...
        # synchronously during place_stop_market, and the SELL branch
        ...
        self._log(
            f"STP-MARKET PLACED: trigger=${stop_price:.2f} "
        ...
        if fill_price is None:
            ...
            # Paper mode fired _on_fill synchronously; the SELL branch in
            # _on_gateway_fill already ran and cleared position state.
            self._log(f"STP-MARKET FILLED @ ${fill_price:.2f}")
        ...
        if ok:
            ...
            if self._audit:
                self._audit.log_order(
                    ...
                    reason=f"parent BUY filled; retarget from initial estimate ${old_stop:.2f}",
                    state_at_time=self._state.value,
                    position_at_time="LONG",
                )
            return
            ...
            try:
                self._audit.log_order(
                    ...
                    reason=(
                        ...
                        f"original child resting at ${old_stop:.2f}. "
    ...
    async def _place_protective_market_fallback(
        ...
        order_id = self._make_engine_id(f"SL_MKT_{OrderSide.SELL.value}", qty)
        ...
        if self._audit:
            self._audit.log_order(
                ...
                order_type="MARKET",
                signal_price=intended_stop_price,
                reason=f"GAP_DOWN_LTP_${ltp:.2f}_INTENDED_STOP_${intended_stop_price:.2f}",
                state_at_time=self._state.value,
                position_at_time="LONG",
            )
        ...
        if self._alerts:
            try:
                ...
                self._alerts.raise_alert(
                    ...
                    severity=AlertSeverity.HIGH,
                    message=(
                        f"Gap-down protective exit on {self.config.ticker}: "
                        f"LTP ${ltp:.2f} ≤ intended stop ${intended_stop_price:.2f} "
                        f"at placement time. Submitted MARKET SELL {qty} "
                        f"instead of STOP-LIMIT (limit ${intended_limit_price:.2f} "
                        f"would not fill into a falling market)."
                    ),
                    context={
                        "ticker": self.config.ticker,
                        "qty": int(qty),
        ...
        self._log(
            f"PROTECTIVE STOP → MARKET FALLBACK: SELL {qty} {self.config.ticker} "
    ...
    def get_status(self) -> dict:
        ...
        status = {
            ...
            # Is the protective SELL stop actually CONFIRMED RESTING at the
            # broker right now? True iff `_pending_stop` carries a SELL
        ...
        # IBKR's per-contract P&L straight from reqPnLSingle (broker truth):
        # realized/unrealized/daily computed by IBKR against its own average
        # cost + mark, correct across multi-day holds. None when unavailable
        # (paper mode, pre-subscription window, or disconnected) — dashboards
        # then fall back to the engine-computed mark-to-market above.
        try:
            get_ibkr_pnl = getattr(self.gateway, 'get_ibkr_pnl', None)
            status["ibkr_pnl"] = get_ibkr_pnl() if callable(get_ibkr_pnl) else None
        except Exception:
            status["ibkr_pnl"] = None
