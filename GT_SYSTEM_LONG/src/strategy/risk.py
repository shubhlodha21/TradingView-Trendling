...
class PortfolioReader:
    ...
    Aggregates produced:
      * `open_notional()`  — Σ across every peer:
                            ...
                            RUNNING-ONLY: pending_notional (working unfilled
                            BUYs) is NOT counted — per operator decision
                            2026-07-02, only filled/running exposure binds
                            against the cap.
    ...
    def _maybe_refresh(self) -> None:
        ...
        try:
            for state_path in self._cwd.glob('.gt_state_*_*.json'):
                ...
                live_notional = 0.0
                # Exposure = RUNNING positions on IBKR only. `pending_notional`
                # (worst-case capital tied up by an unfilled-but-resting BUY)
                # is intentionally NOT booked into the portfolio total — per
                # operator decision 2026-07-02, only filled/running exposure
                # counts toward the $150k cap. (Was previously additive; the
                # tradeoff is that two bots' simultaneous pendings no longer
                # pre-reserve capital, so the combined-fill race is guarded
                # only by each bot's own new-order check below.)
                if live_path.exists():
                    try:
                ...
                if live_notional > 0:
                    ...
                    if not peer_ticker:
                        ...
                        try:
                            ...
                            notional += fallback
                # (pending_notional deliberately omitted — running-only exposure)
        except Exception as e:
...
class PortfolioLimitsReader:
    ...
    100000 --risk 2000`) and consumed by every bot's risk gate.
    ...
    File format:
        {
          ...
          "updated_by": "dashboard_agg.py --exposure 100000"
        }
...
class RiskCheck:
    ...
    def check(self, price: float, qty: int) -> RiskResult:
        ...
        # 2a. Daily loss — hard dollar floor across the WHOLE portfolio.
        # Sum of realized `pnl` from every state file in cwd. Falls back to
        # this instance's cached P&L when PortfolioReader is absent.
        daily_pnl = (
            self.portfolio.daily_pnl()
            if self.portfolio is not None
            else self._cached_daily_pnl
        )
        if daily_pnl <= -eff_max_daily_loss_usd:
            return RiskResult(
                False,
                f"Portfolio daily loss ${daily_pnl:.0f} <= -${eff_max_daily_loss_usd:.0f}",
                circuit_break=True,
            )
    ...
    def record_fill(self, fill_price: float, fill_qty: int, side: str):
        ...
        elif side == "SELL" and self._pending_buy is not None:
            entry_price, entry_qty = self._pending_buy
            matched = min(fill_qty, entry_qty)
            gross = (fill_price - entry_price) * matched
