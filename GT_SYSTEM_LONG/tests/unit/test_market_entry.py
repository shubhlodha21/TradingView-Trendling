...
The entry parent normally rests at IBKR as a BUY STP-LMT and the broker fires
it when LTP rises to the trigger. `--market` is for the case where something
upstream has already detected the crossing and launched this bot in response:
there is no trigger left to wait for, so the parent goes in as a plain MARKET.
...
def _place(gw, **overrides):
    kwargs = dict(
        ...
        parent_limit_price=128.99,
        child_stop_price=128.54,
        parent_order_id="ENTRY_BUY_n1",
        child_order_id="BR_SELL_n1",
    )
    kwargs.update(overrides)
    asyncio.run(gw.place_bracket_buy_stop_market(**kwargs))
...
class TestBracketParentOrderType:
    ...
    def test_default_parent_is_stop_limit(self, gateway):
        ...
        assert parent.lmtPrice == 128.99      # ceiling
    ...
    def test_market_parent_is_a_plain_market_order(self, gateway):
        ...
        assert parent.action == OrderSide.BUY.value
    ...
    def test_protective_child_is_unchanged(self, gateway, market):
        ...
        assert child.action == OrderSide.SELL.value
        assert child.auxPrice == 128.54
    ...
    def test_engine_ids_still_map(self, gateway, market):
        parent, child = _place(gateway, parent_market=market)
        assert gateway._order_id_map[str(parent.orderId)] == "ENTRY_BUY_n1"
        assert gateway._order_id_map[str(child.orderId)] == "BR_SELL_n1"
    ...
    def test_paper_mode_still_takes_the_legacy_path(self):
        ...
        result = asyncio.run(gw.place_bracket_buy_stop_market(
            qty=100, parent_stop_price=128.86, parent_limit_price=128.99,
            child_stop_price=128.54, parent_order_id="E", child_order_id="C",
            parent_market=True,
        ))
