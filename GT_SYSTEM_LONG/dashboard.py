...
class State:
    ...
    __slots__ = (
        ...
        # IBKR per-contract P&L (reqPnLSingle) when available — the broker's
        # own realized/unrealized/daily. `pnl_source` is 'IBKR' when the
        # displayed real/unreal come straight from the broker, or 'est' when
        # we fell back to the engine mark-to-market (paper, pre-subscription,
        # or disconnected). `ibkr_daily_pnl` is IBKR's today-only figure,
        # useful on multi-day holds where unrealized is cumulative-since-open.
        'ibkr_pnl', 'ibkr_daily_pnl', 'pnl_source',
        'wins', 'losses',
    ...
    def sync_engine(self):
        ...
        #   unreal  = (current - entry) × qty       — pure mark-to-market
        ...
        # ── IBKR per-contract P&L override (broker truth) ─────────────────
        # When reqPnLSingle has reported for this contract, PREFER IBKR's own
        # realized/unrealized over the engine mark-to-market computed above.
        # IBKR prices against its average cost + mark and stays correct across
        # multi-day holds (overnight carry, daily marks, cost-basis drift) —
        # exactly the cases where the engine estimate diverges from TWS. We
        # keep the estimate as the fallback (paper, pre-subscription window,
        # disconnected) and flag which source is live via `pnl_source`.
        self.ibkr_pnl = s.get('ibkr_pnl')
        self.ibkr_daily_pnl = None
        self.pnl_source = 'est'
        if self.ibkr_pnl:
            ib_real = self.ibkr_pnl.get('realized_pnl')
            ib_unreal = self.ibkr_pnl.get('unrealized_pnl')
            self.ibkr_daily_pnl = self.ibkr_pnl.get('daily_pnl')
            # Only switch a field over when IBKR actually reported it (its
            # NaN sentinels are coerced to None upstream). Realized and
            # unrealized are handled independently so a half-populated first
            # push still shows whatever IBKR has given us.
            if ib_real is not None or ib_unreal is not None:
                self.pnl_source = 'IBKR'
                if ib_unreal is not None:
                    self.unrealized_pnl = ib_unreal
                if ib_real is not None:
                    self.realized_pnl = ib_real
                self.total_pnl = self.realized_pnl + self.unrealized_pnl
    ...
    def position(self) -> str:
        if not self.engine:
            return "FLAT"
        return "LONG" if self.engine.get_status().get('position_open') else "FLAT"
...
def draw_state_panel(s: State) -> list[str]:
    ...
    sc = sc_map.get(s.engine_state, "")
    pos = f"{G}LONG{R} @${s.entry:.2f}" if s.position == "LONG" and s.entry else f"{D}FLAT{R}"
...
def draw_risk_panel(s: State) -> list[str]:
    ...
    # real/unreal are IBKR's reqPnLSingle figures when available (broker
    # truth, `pnl_source == 'IBKR'`), else the engine mark-to-market estimate.
    # sync_engine already applied the override + the open-cycle entry-
    # commission deduction, so we just read the pre-computed fields here.
    ...
    if getattr(s, 'ibkr_daily_pnl', None) is not None:
        day_c = G if s.ibkr_daily_pnl >= 0 else R_
        lines.append(f"  Today:  {day_c}${s.ibkr_daily_pnl:+.2f}{R}")
    lines += [
...
def draw_v2_portfolio_footer(s: State, width: int) -> list[str]:
    ...
    if s.position == "LONG" and s.entry > 0 and s.feed.last > 0:
...
def draw_v2_position_ladder(s: State, width: int) -> list[str]:
    ...
    if s.highest_price > ltp:
        levels.append((s.highest_price, "PEAK", Q.BLUE))
    if s.position == "LONG" and s.entry > 0:
    ...
    if s.engine_state == "WAITING_REENTRY" and s.breakout_level > 0:
        levels.append((s.breakout_level, "REBREAKOUT", Q.YELLOW))
    elif s.trigger > 0:
...
def draw_v2_system_status(s: State, width: int) -> list[str]:
    ...
    if s.position == "LONG" and s.entry > 0:
        out.append(
            f"  {Q.GRAY_3}Position{Q.RESET}  {Q.GREEN}LONG{Q.RESET} "
    ...
    # real/unreal come from IBKR's reqPnLSingle when it has reported for this
    # contract (`pnl_source == 'IBKR'`) — broker truth, correct on multi-day
    # holds. Otherwise they're the engine mark-to-market estimate, tagged
    # 'est' so the operator knows the number is modeled, not broker-confirmed.
    realized = s.realized_pnl
    ...
    src = getattr(s, 'pnl_source', 'est')
    src_c = Q.GREEN if src == 'IBKR' else Q.YELLOW
    out.append(f"  {Q.GRAY_3}P&L{Q.RESET}  {src_c}{src}{Q.RESET}")
    ...
    # IBKR's today-only figure — distinct from `unreal` (cumulative since the
    # position opened) on a multi-day hold. Only shown when IBKR reported it.
    if getattr(s, 'ibkr_daily_pnl', None) is not None:
        day_c = Q.GREEN if s.ibkr_daily_pnl >= 0 else Q.RED
        out.append(f"    {Q.GRAY_3}today{Q.RESET}   {day_c}${s.ibkr_daily_pnl:+7.2f}{Q.RESET}")
...
def build_frame(s: State, footer_override: Optional[str] = None) -> str:
    ...
    # No blank separator between SYSTEM and CONNECTION — each panel's
    # header acts as its own visual divider, saves a row of vertical space.
    right = sys_block + conn_block
