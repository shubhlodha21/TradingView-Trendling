...
def render_summary(watchers: list[SymbolWatcher]) -> str:
    ...
    for w in live_watchers + offline_watchers:
        ...
        if account_equity and account_bp:
            break
    ...
    #
    # RUNNING-ONLY exposure: we count filled/running positions only and do
    # NOT add `pending_notional` (working unfilled BUYs). This mirrors
    # PortfolioReader.open_notional() on the risk-gate side, which was
    # changed to running-only per operator decision 2026-07-02 — so the
    # displayed exposure matches what the gate actually checks against the
    # $150k cap.
    gross_notional = 0.0
    ...
    def _render_row(w, idx, *, offline: bool) -> str:
        ...
        # FX prices are quoted in the pair's QUOTE currency, not USD — a
        # USDJPY price is ¥162, NOT $162 (that ambiguity caused a sizing
        # mix-up). Pick the right symbol per row: equity / *USD pairs → "$";
        # FX → the quote-ccy symbol. (Prices only — P&L/equity stay USD.)
        _q = (w.symbol or "").upper()
        if len(_q) == 6 and _q.isalpha():
            cur = {"USD": "$", "JPY": "¥", "EUR": "€", "GBP": "£", "CHF": "Fr",
                   "AUD": "A$", "CAD": "C$", "NZD": "NZ$"}.get(_q[3:], _q[3:] + " ")
        else:
            cur = "$"
        ...
        if offline:
            ...
            pos_str = (
                f"{Q.GRAY_4}LONG {qty} @ {cur}{entry:.2f}{Q.RESET}"
            ...
            cells = [
                ...
                f"{Q.GRAY_4}{cur}{trig:.2f}{Q.RESET}",
            ]
        else:
            state_c = _STATE_COLORS.get(state, Q.GRAY_3)
            if pos_open and entry > 0:
                pos_str = (
                    f"{Q.GREEN}LONG{Q.RESET} {Q.WHITE}{qty}{Q.RESET} @ "
                    f"{Q.WHITE}{cur}{entry:.2f}{Q.RESET}"
                )
            else:
            ...
            if pos_open and last > 0 and entry > 0:
                ...
                unreal = 0
            # ── IBKR override (broker truth) ─────────────────────────────
            # run_live forwards reqPnLSingle into the live snapshot. When
            # present, PREFER IBKR's realized/unrealized over the estimates
            # above — correct on multi-day holds (carry/marks/cost-basis).
            # Per-field so a half-populated first push still upgrades what
            # IBKR has given us; missing fields keep the estimate.
            ibkr_pnl = live.get('ibkr_pnl') or None
            if ibkr_pnl:
                ib_real = ibkr_pnl.get('realized_pnl')
                ib_unreal = ibkr_pnl.get('unrealized_pnl')
                if ib_unreal is not None:
                    unreal = ib_unreal
                if ib_real is not None:
                    pnl = ib_real
            last_str = f"{cur}{last:.2f}" if last > 0 else "--"
            unr_c = Q.GREEN if unreal >= 0 else Q.RED
            pnl_c = Q.GREEN if pnl >= 0 else Q.RED

            # POSVAL for live: RUNNING-ONLY — position_notional (qty × LTP
            # for the already-filled position). Working unfilled BUYs
            # (pending_notional) are NOT included, matching the risk-gate
            # and the running-only exposure total (operator decision
            # 2026-07-02), so per-row POSVALs sum to TOTAL ACTIVE EXPOSURE.
            # A resting-but-unfilled BUY renders as $0 here until it fills.
            ...
            cells = [
                ...
                f"{Q.YELLOW}{cur}{trig:.2f}{Q.RESET}",
            ]
    ...
    if not watchers:
        ...
        # Sum of position_notional across LIVE watchers ONLY — RUNNING-ONLY
        # exposure. Working unfilled BUYs (`pending_notional`) are NOT
        # counted, matching the portfolio risk gate which was changed to
        # running-only per operator decision 2026-07-02, so this number and
        # the gate's "combined exposure" stay aligned.
        #
        ...
        # "Contributing" count = bots with a RUNNING position open. Pending
        # BUYs no longer count toward exposure, so they don't count here.
...
def populate_drilldown_state(watcher: SymbolWatcher) -> State:
    ...
    if st.get('position_open') and state.feed.last > 0 and st.get('entry_price'):
        qty = st.get('quantity', 0) or 0
        state.unrealized_pnl = (state.feed.last - st['entry_price']) * qty
    ...
    # ── IBKR override (broker truth) ─────────────────────────────────────
    # Forwarded by run_live into the live snapshot via reqPnLSingle. Prefer
    # it over the estimates above so the drill-down SYSTEM panel matches the
    # per-ticker terminal (and TWS) on multi-day holds. Sets pnl_source /
    # ibkr_daily_pnl so dashboard.py's shared render shows the source tag +
    # today's figure. Mirrors the same-named logic in dashboard.py.sync_engine.
    state.ibkr_pnl = live.get('ibkr_pnl') or None
    state.ibkr_daily_pnl = None
    state.pnl_source = 'est'
    if state.ibkr_pnl:
        ib_real = state.ibkr_pnl.get('realized_pnl')
        ib_unreal = state.ibkr_pnl.get('unrealized_pnl')
        state.ibkr_daily_pnl = state.ibkr_pnl.get('daily_pnl')
        if ib_real is not None or ib_unreal is not None:
            state.pnl_source = 'IBKR'
            if ib_unreal is not None:
                state.unrealized_pnl = ib_unreal
            if ib_real is not None:
                state.realized_pnl = ib_real
            state.total_pnl = state.realized_pnl + state.unrealized_pnl
