...
class LiveTrader:
    ...
    __slots__ = ('config', 'gateway', 'conn_mgr', 'feed_handler',
                 ...
                 # `dashboard_agg.py --exposure 100000`) and every bot
    ...
    async def setup_feed(self) -> ProductionFeed:
        ...
        # Quote-driven assets (index/FX CFDs, spot FX) never print a trade,
        # so `last` stays 0. Tell the pipeline to forward their bid/ask-only
        # ticks to the strategy; otherwise the engine gets NO ticks (no
        # on_tick, no heartbeat → "Price stale" blocks, no feed.csv). Equities
        # keep last>0 ticks, so this stays False for them (no behaviour change).
        quote_driven = False
        try:
    ...
    def _print_naked_position_banner(self, err: 'NakedPositionError') -> None:
        ...
        print(f"{R_}{B}║{R} that position. Starting the bot would risk DOUBLE EXPOSURE on the next breakout.")
        print(f"{R_}{B}║{R}")
    ...
    async def start(self):
        ...
        if spec is not None:
            ...
            stop_est = round(stop_est, dec)
        print(f"  Stop: ${stop_est:.{dec}f} ({self.config.stop_loss_pct * 100:.1f}%)")
    ...
    def write_live_snapshot(self) -> None:
        ...
        try:
            ...
            # entry resting at IBKR that hasn't filled. As of 2026-07-02
            # this is INFORMATIONAL ONLY — exposure is running-only, so a
            # resting BUY no longer counts toward the portfolio cap on
            # either the risk-gate or dashboard side. We still compute and
            # write the raw number so tooling can surface "a BUY is resting".
            #
            ...
            # Two utilization ratios, pre-computed. RUNNING-ONLY: based on
            # position_notional (filled/running position) only — working
            # unfilled BUYs (pending_notional) are NOT counted, matching the
            # risk gate and dashboards (operator decision 2026-07-02). The
            # raw pending_notional is still written to the snapshot below as
            # an informational field, but it no longer drives exposure/BP.
            ...
            # ── IBKR per-contract P&L (broker truth) ──────────────────────
            # Straight from reqPnLSingle via the gateway cache. The aggregator
            # doesn't share this process's IBKR socket, so we forward the
            # broker's own realized/unrealized/daily here for it to display
            # in preference to the mark-to-market estimate. None (dict absent
            # or per-field None) means IBKR hasn't reported yet → aggregator
            # falls back to the computed value, same as the per-ticker view.
            ibkr_pnl = None
            try:
                gw = getattr(self.engine, 'gateway', None)
                getter = getattr(gw, 'get_ibkr_pnl', None) if gw else None
                if callable(getter):
                    ibkr_pnl = getter()
            except Exception:
                ibkr_pnl = None
            ...
            snap = {
                ...
                # Worst-case capital outlay of a working unfilled BUY.
                # INFORMATIONAL ONLY as of 2026-07-02 — exposure is now
                # running-only, so neither the risk gate nor the dashboards
                # count this toward the portfolio cap. Kept in the snapshot
                # so tooling can still surface "a BUY is resting" if useful.
                ...
                # IBKR per-contract P&L (reqPnLSingle) — broker truth. dict
                # of daily_pnl/unrealized_pnl/realized_pnl/position/value (any
                # may be None) or None if unavailable. Consumed by
                # dashboard_agg.py in preference to its own mark-to-market.
                'ibkr_pnl': ibkr_pnl,
            }
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='SELL exit buffer = this fraction of (entry-stop) distance (default 0.05 = 5%%). '
             'Scales the limit buffer with how far your stop sits below entry. Set 0 to disable.'
    )
    parser.add_argument(
        '--offset-entry-pct', type=float,
        help='BUY entry buffer = this fraction of trigger price (default 0.0005 = 5 bps). '
    ...
    parser.add_argument(
        ...
        help='Enter with a MARKET order instead of resting a BUY STP-LMT at '
             '--trigger. For the case where something upstream (e.g. the RTH '
             'trendline runner) has ALREADY seen the crossing and launched this '
             'bot in response: the trigger has been met, so there is nothing '
             'left to wait for, and a limit the tape has already passed would '
             'never fill. --trigger is still required — the protective child '
             'stop is sized from it until the real fill price is known. '
             'WARNING: a market order has no price ceiling. It fills at '
             'whatever the book offers when it lands, which can be worse than '
             '--trigger if the market moved while this bot was connecting. '
             '--offset-entry-pct is ignored in this mode. Default off.')
    parser.add_argument(
        '--cfd', action='store_true',
        help='Trade the CFD contract for this symbol instead of equity/spot. '
             'Resolves the CFD variant (e.g. NVDA share-CFD, not NVDA equity; '
             'IBUS500 index-CFD; EURUSD FX-CFD). Default off → equity/FX as before.')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Override portfolio combined-exposure cap (in USD). The engine '
             ...
             'exceed <EXPOSURE>. Default is $100000 (from '
             ...
             '`python dashboard_agg.py --exposure 100000` — every running bot '
