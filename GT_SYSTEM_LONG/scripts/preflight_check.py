...
def check_exposure_cap() -> tuple[bool, str, str | None]:
    """The $150k cap must be in models.py defaults (lowered from $200k 2026-07-02)."""
    models_py = REPO_ROOT / "src" / "config" / "models.py"
    text = models_py.read_text()
    for line in text.splitlines():
        # only the dataclass default line (a literal assignment), not the env-
        # fallback line which also contains the number inside a string.
        if "max_position_value_usd:" in line and "=" in line:
            if "150000" in line:
                return True, f"Exposure cap = 150000 ({line.strip()})", None
            return False, f"Exposure cap line: {line.strip()}", "Set default to 150000.0 in models.py"
...
def main():
    ...
    checks = [
        ...
        ("Exposure cap = $150k",                   check_exposure_cap),
