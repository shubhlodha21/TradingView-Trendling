...
TABS:
    ...
    3. Open Positions    LONGs still held at session end with unrealized
                         P&L, SL distance, peak, hold duration.
...
class OpenPositionsTab(TabBuilder):
    ...
    def build(self) -> None:
        ...
        row = self.add_sheet_title(
            ...
            "Open Positions",
            f"{self.date_str}  ·  {self.account}  ·  "
            f"LONGs still held at session close — entry, peak, stop, distance from stop",
        )
        ...
        headers = [
            "Ticker", "Strategy", "Qty", "Entry VWAP", "Peak Price",
        ...
        for t, d in sorted(opens.items()):
            entry = d["entry_price"]
            stop = d["stop_loss"]
            dist_dollar = entry - stop if entry and stop else 0
            ...
            rows.append([
                t, d["strategy"], d["qty"], entry, d["peak"], stop,
    ...
    def _find_open_positions(self) -> Dict[str, Dict]:
        ...
        for t, rows in per_ticker.items():
            ...
            out[t] = {
                ...
                "peak": self.loader.f(last.get("highest_price")),
